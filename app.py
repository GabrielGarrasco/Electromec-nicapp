...
st.set_page_config(page_title="Electromecánica", layout="wide")
cargar_css()
...
def dialog_nueva_materia_plan():
    ...
    if st.button("Guardar en el Plan", type="primary", use_container_width=True):
        if nombre:
            ...
            st.error("⚠️ Falta el nombre de la materia.")
...
def dialog_nueva_meta():
    ...
    if not nombres_materias:
        st.warning("⚠️ Primero agregá materias activas desde el menú 'Organización'.")
        return
...
def dialog_editar_meta(meta_idx):
    ...
    if not nombres_materias:
        st.warning("⚠️ No hay materias activas.")
        return
...
def dialog_nueva_materia_activa():
    ...
    if not opciones_disponibles:
        st.warning("⚠️ No tenés materias en estado 'Cursando' o 'Regular' disponibles para agregar.")
        return
...
def dialog_agregar_sesion():
    nombres_materias = [m["nombre"] for m in st.session_state['materias']]
    if not nombres_materias:
        st.warning("⚠️ Agregá materias en 'Organización' para poder registrar sesiones.")
        return
